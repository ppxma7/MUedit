...
function [PulseT, distime] = batchprocessfilters_penalty(MUFilters, wSIG, coordinates, exFactor, differentialmode, ltime, fsamp)
...
for nwin = 1:size(wSIG,2)
    for j = 1:size(MUFilters{nwin},2)
        ...
        maxSpikes = maxk(PulseT(MUnb,spikes),8*nr_win);
        PulseT(MUnb,:) = PulseT(MUnb,:)/mean(maxSpikes(2*nr_win+1:8*nr_win)); % normalization of the MU pulse train
        spikes = spikes(PulseT(MUnb,spikes)<1.2);
