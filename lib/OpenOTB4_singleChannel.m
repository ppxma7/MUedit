...
xmlFiles = {'Tracks_000.xml', 'Tracks_001.xml', 'Tracks_002.xml'};
TI = {};
for xi = 1:length(xmlFiles)
    xmlPath = fullfile('tmpopen', xmlFiles{xi});
    if exist(xmlPath, 'file')
        ab = xml2struct(xmlPath);
        ti = ab.ArrayOfTrackInfo.TrackInfo;
...
for n = 1:nTrack
    ...
    if strncmpi(Title{n}, 'HD', 2) || strncmpi(Title{n}, 'GR', 2) || contains(Title{n}, 'Muovi', 'IgnoreCase', true)
        isEMG(n) = true;
        signal.gridname{end+1} = Title{n};
    end

    if contains(Title{n}, 'acquired data', 'IgnoreCase', true) || ...
            contains(Title{n}, 'performed path', 'IgnoreCase', true) || ...
            contains(Title{n}, 'requested path', 'IgnoreCase', true)
        isForce(n) = true;
    end
...
if strcmp(device, 'Novecento+')
    ...
    sigMask = cellfun(@(p) ~isempty(regexp(p, '\.sig$')), Path);
    TotCh = sum(cell2mat(nChannel(sigMask)));
    ...
    for n = 1:nTrack
        ...
        if ~contains(Path{n}, '.sig'); continue; end  % ADD THIS
